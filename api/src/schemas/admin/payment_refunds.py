...
기존 `admin/refunds.py`는 폐지 예정인 사용자 자가 환불 폼 → `manual_refund_queue` 기반
구식 흐름이다. 본 모듈은 v1.1 신규 흐름 — 관리자가 결제 건당 환불 금액을 직접 입력하는
부분/전액 환불 — 을 위해 분리했다. 두 흐름의 import 경로를 명확히 분리해 향후 구식
경로 제거 시 정리 부담을 줄이는 것이 목적.
