...
def _apply_block(user: User, block_action: BlockActionRequest, now: datetime) -> None:
    """user를 전체 계정 차단 상태로 갱신 — subscription_status='blocked'."""
    ...
    # 차단 즉시 기존 세션을 무효화 — 활성 쿠키의 sid 가 더 이상 매칭되지 않아
    # 다음 요청에서 401 AUTH_SESSION_SUPERSEDED 로 자동 로그아웃된다. 단, login 흐름은
    # 401 대신 새로운 ACCOUNT_BLOCKED(403) 안내로 거부한다(auth_service.login_user 참고).
...
async def update_permission(
    ...
    if payload.unblock is True:
        _apply_unblock(user)
        # NOTE: actioned anomaly 이력은 그대로 보존한다. 이상탐지 리스트에서 사라지지 않게.
        # 현재 적용 여부는 상세 드로어가 사용자 차단 필드로 직접 확인한다.
