...
describe("useQAStore — markReframe 액션 (Story 2.6)", () => {
  const ASSISTANT_ID = "asst-reframe";

  beforeEach(() => {
    useQAStore.getState().addMessage({
      id: ASSISTANT_ID,
      role: "assistant",
      content: "어느 치아인지 알려주실래요?",
      status: "complete",
      timestamp: "2026-01-01T00:00:00.000Z",
    });
  });

  it("markReframe이 reframe 필드를 설정하고 content를 followUpQuestion으로 교체한다", () => {
    const payload = {
      followUpQuestion: "어느 치아인지 알려주세요?",
      options: ["상악 전치", "하악 구치", "어금니"],
    };
    useQAStore.getState().markReframe(ASSISTANT_ID, payload);

    const msg = useQAStore.getState().messages.find((m) => m.id === ASSISTANT_ID);
    expect(msg?.reframe).toEqual(payload);
    expect(msg?.content).toBe("어느 치아인지 알려주세요?");
  });

  it("markReframe 후 content와 reframe.followUpQuestion이 동일하다 (UI/DB SoT)", () => {
    const payload = {
      followUpQuestion: "치아 위치를 알려주세요?",
      options: ["1번 치아", "2번 치아", "3번 치아"],
    };
    useQAStore.getState().markReframe(ASSISTANT_ID, payload);

    const msg = useQAStore.getState().messages.find((m) => m.id === ASSISTANT_ID);
    expect(msg?.content).toBe(msg?.reframe?.followUpQuestion);
  });

  it("markReframe이 다른 ID의 메시지를 수정하지 않는다", () => {
    useQAStore.getState().addMessage({
      id: "other",
      role: "user",
      content: "사용자 메시지",
    ...
    useQAStore.getState().markReframe(ASSISTANT_ID, {
      followUpQuestion: "질문?",
      options: ["가", "나", "다"],
    });
    const other = useQAStore.getState().messages.find((m) => m.id === "other");
    expect(other?.reframe).toBeUndefined();
    expect(other?.content).toBe("사용자 메시지");
  });
});
