...
type SegmentKey = "free" | "pro" | "blocked" | "withdrawn";

/**
 * 각 segment 클릭 시 /admin/users 검색 필터로 매핑.
 * 무료/Pro/차단은 대시보드 카운트가 withdrawn_at IS NULL 을 강제하므로
 * withdrawn=false 를 함께 넘겨야 사용자 페이지 결과가 동일 개수로 나온다.
 */
function userListHrefForSegment(seg: SegmentKey): string {
  switch (seg) {
    case "free":
      return "/admin/users?subscription_status=free&withdrawn=false";
    case "pro":
      return "/admin/users?subscription_status=pro&withdrawn=false";
    case "blocked":
      return "/admin/users?subscription_status=blocked&withdrawn=false";
    case "withdrawn":
      return "/admin/users?withdrawn=true";
  }
}
...
export function SubscribersDonut({ data }: Props) {
  ...
  const segments = useMemo(
    () => [
      ...
      { key: "pro" as SegmentKey, name: SEGMENT_LABELS.pro, value: data.pro_count },
      { key: "blocked" as SegmentKey, name: SEGMENT_LABELS.blocked, value: data.blocked_count },
      { key: "withdrawn" as SegmentKey, name: SEGMENT_LABELS.withdrawn, value: data.withdrawn_count },
    ],
    [data],
  );
        ...
        {segments.map((seg, idx) => {
          ...
          return (
            <li key={seg.name} className={styles.legendItem}>
              <Link
                href={userListHrefForSegment(seg.key)}
                className={styles.legendLink}
                aria-label={`${seg.name} 사용자 목록 보기 — ${seg.value.toLocaleString()}명`}
              >
                <span
      ...
      <div className={styles.kpiRow}>
        {segments.map((seg) => (
          <Link
            ...
            aria-label={`${seg.name} 사용자 목록 보기 — ${seg.value.toLocaleString()}명`}
          >
            <KPICard label={seg.name} value={fmt(seg.value)} />
          </Link>
        ))}
      </div>
    </section>
