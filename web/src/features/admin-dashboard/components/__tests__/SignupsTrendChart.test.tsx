...
describe("SignupsTrendChart", () => {
  it("3 series 라벨(누적/활성/탈퇴) 렌더", () => {
    const data = makeResponse([
      { bucket_start: "2026-01-01", cumulative: 5, active: 4, withdrawn: 1 },
      { bucket_start: "2026-02-01", cumulative: 8, active: 6, withdrawn: 2 },
    ]);
  ...
  it("aria-label 동적 문구가 마지막 버킷 값을 반영", () => {
    const data = makeResponse([
      { bucket_start: "2026-01-01", cumulative: 1, active: 1, withdrawn: 0 },
      { bucket_start: "2026-02-01", cumulative: 7, active: 5, withdrawn: 2 },
    ]);
    render(<SignupsTrendChart data={data} />);
    const region = screen.getByRole("img");
    expect(region.getAttribute("aria-label")).toBe(
      "가입자 추세 — 누적 7, 활성 5, 탈퇴 2",
    );
  });
  ...
  it("모든 값 0 → EmptyState 렌더 (차트 미렌더)", () => {
    const data = makeResponse([
      { bucket_start: "2026-04-01", cumulative: 0, active: 0, withdrawn: 0 },
    ]);
    render(<SignupsTrendChart data={data} />);
    expect(screen.getByRole("status")).toBeTruthy();
  });

  it("KPICard 3개 — 마지막 버킷 값 표시", () => {
    const data = makeResponse([
      { bucket_start: "2026-01-01", cumulative: 100, active: 80, withdrawn: 20 },
    ]);
    render(<SignupsTrendChart data={data} />);
