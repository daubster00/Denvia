 ...
 * SSE는 단순한 토큰→done 시퀀스로 응답하여 messages.length 0 → ≥1 전환만 검증.
 */
...
test.describe("Story 2.7 — Chat-First 자동 전환", () => {
  ...
  test("AC-1: /chat 진입 시 messages=0 → hero 활성 textarea, 메시지 영역 미렌더", async ({ page }) => {
  ...
  test("AC-1: 메시지를 직접 주입하여 hero → inline 전환 검증 (메시지 영역 노출)", async ({ page }) => {
    await page.goto("/chat");
